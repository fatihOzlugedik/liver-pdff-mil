...
class LiverPDFFDataset(Dataset):
    ...
    def _sample_indices(self, T: int) -> np.ndarray:
        if self.split == "train":
            # Stratified temporal sampling: divide video into n_frames equal
            # intervals and sample one random frame from each interval.
            # This ensures balanced coverage across the entire video duration
            # while still introducing stochasticity each epoch.
            n = self.n_frames
            if T >= n:
                ...
                # Fewer frames than requested: sample all, then fill remaining
                # intervals by random resampling (with replacement)
                base = np.arange(T)
