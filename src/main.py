...
def train_one_fold(base_cfg: CFG, fold_idx: int | None):
    ...
    # Fixed seed across all folds and all aggregators — ensures every model
    # sees the exact same frame sequence per epoch, enabling fair comparison.
    set_seed(cfg.seed)
...
def main():
    ...
    base_cfg = CFG(
        ...
        # Pass these BEFORE __post_init__ runs so tag and config.json are correct
        img_size     = args.img_size  if args.img_size  is not None else 384,
        n_frames     = args.n_frames  if args.n_frames  is not None else 175,
    )
